...
if __name__ == '__main__':
    ...
    model = LogisticRegression(class_weight='balanced', penalty="l1", solver='liblinear', C=0.01)
    model.fit(X, y)
    ...
    for class_index in range(model.coef_.shape[0]):
        top10_indices = np.argsort(model.coef_[class_index])[-10:]
        top10_terms = [vocab[i] for i in top10_indices]
        print(f"Top 10 characteristic words for class {class_index}: {top10_terms}")
